...
class PatientSimulator:
    ...
    def _apply_deterioration(self, vitals: Dict[str, float]) -> Dict[str, float]:
        ...
        Resp rate    → rises (breathing faster to compensate)
        """
        ...
        modified["temperature"] += (
            self.deterioration_severity * 3.0 
            + self.rng.normal(0, 0.1 * noise_scale)
        )
        modified["heart_rate"] += (
            self.deterioration_severity * 50.0 
            + self.rng.normal(0, 2.0 * noise_scale)
        )
        modified["systolic_bp"] -= (
            self.deterioration_severity * 30.0 
            + self.rng.normal(0, 3.0 * noise_scale)
        )
        modified["diastolic_bp"] -= (
            self.deterioration_severity * 20.0 
            + self.rng.normal(0, 2.0 * noise_scale)
        )
        modified["spo2"] -= (
            self.deterioration_severity * 6.0 
            + self.rng.normal(0, 0.5 * noise_scale)
        )
        modified["respiratory_rate"] += (
            self.deterioration_severity * 10.0 
    ...
    """
        Without this, vitals could jump from 75 to 115 in one step — unrealistic. 
        This blends 70% of the old value with 30% of the new target. Creates natural-looking gradual changes.
    """
    ...
    def tick(self):
        ...
        This is the most important function. Call it once = one timestep passes.
        ```
        ...
        # Store for next iteration and for observation
...
if __name__ == "__main__":
    ...
    # Test 2: Hypertensive patient (Task 1)
    print("\n[TEST 2] Hypertensive Patient - Baseline BP ~150/95")
    print("-" * 60)
    ...
    for i in range(1, 6):
        ...
        print(f"Step {i:02d} | BP: {vitals['systolic_bp']:5.1f}/{vitals['diastolic_bp']:5.1f} "
              f"(This is NORMAL for this patient!)")
    ...
    for i in range(0, 361, 60):
        # Manually jump timestep for demo purposes only.
        # This bypasses normal tick progression to simulate sampling at hourly intervals
        # without running all 60 intermediate ticks. Do NOT do this in real training.
